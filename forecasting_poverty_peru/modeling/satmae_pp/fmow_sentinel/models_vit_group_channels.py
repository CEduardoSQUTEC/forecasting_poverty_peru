...
from forecasting_poverty_peru.utils.pos_embed import get_2d_sincos_pos_embed
from forecasting_poverty_peru.utils.pos_embed import get_1d_sincos_pos_embed_from_grid
...
class GroupChannelsVisionTransformer(VisionTransformer):
    ...
    def __init__(self, global_pool=False, channel_embed=256,
        ...
        self.pos_embed = nn.Parameter(torch.zeros(
            1, num_patches + 1, embed_dim - channel_embed))
    ...
    def forward_features(self, x):
        ...
        pos_channel = torch.cat(
            (pos_embed, channel_embed), dim=-1)  # (1, c, L, D)
        ...
        cls_pos_channel = torch.cat(
            (self.pos_embed[:, :1, :], self.channel_cls_embed), dim=-1)  # (1, 1, D)
